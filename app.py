...
.upload-wrap{background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.06);border-radius:22px;padding:1.5rem;margin:0 auto 2rem;}
...
.cta-box{background:linear-gradient(135deg,rgba(200,255,87,.08),rgba(200,255,87,.02));border:1px solid rgba(200,255,87,.2);border-radius:20px;padding:3rem;text-align:center;margin:2rem 0;}
.cta-title{font-family:'DM Serif Display',serif;font-size:2rem;color:#f4f1eb;margin-bottom:.75rem;}
.cta-sub{color:#6b6b7a;font-size:14px;margin-bottom:2rem;font-weight:300;max-width:500px;margin-left:auto;margin-right:auto;}
...
INDUSTRY_BENCHMARKS = {
    "restaurant": {"Food Cost %":(38,28,"lower"),"Staff Cost %":(30,25,"lower"),"Rent %":(10,8,"lower"),"Net Margin %":(12,18,"higher")},
    "clinic": {"Doctor Salary %":(45,38,"lower"),"Lab Revenue Mix %":(25,35,"higher"),"Overdue %":(8,3,"lower"),"Net Margin %":(18,25,"higher")},
    "retail": {"Inventory Cost %":(60,50,"lower"),"Shrinkage %":(3,1.5,"lower"),"Staff Cost %":(12,10,"lower"),"Net Margin %":(10,15,"higher")},
    "agency": {"Salary % of Revenue":(55,42,"lower"),"Freelancer Cost %":(15,10,"lower"),"Software Cost %":(8,5,"lower"),"Net Margin %":(22,30,"higher")},
}
...
def parse_uploaded_file(file):
    try:
        ...
        df["Type"] = df["Type"].replace({"Dr":"Expense","Cr":"Sales","Debit":"Expense","Credit":"Sales",
            "Out":"Expense","In":"Sales","Payment":"Expense","Receipt":"Sales","Purchase":"Expense","Sale":"Sales"})
        ...
        return df, True, f"Loaded {len(df)} transactions successfully", min(60+len(mapped)*5,95), mapped
    except Exception as ex:
        return None, False, str(ex), 0, []
...
def compute_health_score(df):
    ...
    if len(s)>0:
        tc=s.groupby("Party")["Amount"].sum().max(); conc=(tc/rev*100) if rev>0 else 0
...
def generate_owner_summary(df, industry):
    ...
    opening=(f"This business generated <strong>{fmt_inr(profit)}</strong> in profit on <strong>{fmt_inr(rev)}</strong> revenue"
             if profit>=0 else f"This business is running at a <strong>loss of {fmt_inr(abs(profit))}</strong> on <strong>{fmt_inr(rev)}</strong> revenue")
    return (f"{opening}, with a <strong>{margin:.1f}% margin</strong>. Biggest cost: <strong>{top_exp}</strong> at "
            f"<strong>{fmt_inr(top_exp_amt)}</strong>. Strongest revenue contributor: <strong>{top_cust}</strong> at "
            f"<strong>{fmt_inr(top_cust_amt)}</strong>. Cash stuck in overdue invoices: <strong>{fmt_inr(overdue)}</strong>. "
            f"The fastest improvement right now: better collections and tighter control of {top_exp}.")
...
def month_vs_month(df):
    ...
    cr=sm.get(cur_m,0); pr=sm.get(prev_m,0)
    ce=em.get(cur_m,0); pe=em.get(prev_m,0)
    cp=cr-ce; pp=pr-pe
    def pct(c,p): return ((c-p)/p*100) if p!=0 else 0
    return {"cur_month":cur_m,"prev_month":prev_m,
            "rev":(cr,pr,pct(cr,pr)),"exp":(ce,pe,pct(ce,pe)),"profit":(cp,pp,pct(cp,pp))}
...
def get_top_3_problems(df, industry):
    ...
    else: problems.append({"severity":"green","title":"Profitability looks healthy","text":f"Margin is <strong>{margin:.1f}%</strong> — a solid foundation.","action":f"Protect this by keeping <strong>{top_exp}</strong> under control and growing repeat revenue."})
    if overdue>0:
        sev="red" if op>10 else "amber"
        problems.append({"severity":sev,"title":"Cash stuck in overdue invoices","text":f"<strong>{fmt_inr(overdue)}</strong> overdue — <strong>{op:.1f}%</strong> of revenue outside your business.","action":"Follow up on collections this week. This is the fastest cash flow fix."})
    ...
    if top_cust_pct>35: problems.append({"severity":"amber","title":"Too dependent on one customer","text":f"<strong>{top_cust}</strong> is <strong>{top_cust_pct:.1f}%</strong> of your revenue — risky concentration.","action":"Add 2–3 more active revenue sources this quarter."})
    else: problems.append({"severity":"amber" if top_exp_pct>28 else "green","title":f"{top_exp} is your biggest cost","text":f"Accounts for <strong>{top_exp_pct:.1f}%</strong> of all expenses.","action":"Audit this category line by line — highest-impact place to improve profitability."})
    return problems[:3]
...
def get_this_week_actions(df, industry):
    ...
    overdue=df[df["Status"]=="Overdue"]["Amount"].sum() if "Status" in df.columns else 0
    te=e.groupby("Category")["Amount"].sum() if len(e)>0 else pd.Series(dtype=float)
    top_exp=te.idxmax() if len(te)>0 else "largest cost"
    actions=[]
    if overdue>0: actions.append(f"Follow up on <strong>{fmt_inr(overdue)}</strong> in overdue invoices — call the top 3 overdue parties today.")
    ...
    actions.append("Spend 15 minutes verifying this month's revenue has actually turned into cash.")
    if industry=="clinic": actions.append("Check if consultations are converting into lab tests or higher-value services.")
    elif industry=="retail": actions.append("Identify slow-moving inventory and clear dead stock before reordering.")
    elif industry=="agency": actions.append("Review if team salaries and freelancer spend match actual billable work.")
    return actions[:5]
...
def get_profit_leaks(df, industry):
    ...
    if len(s)>0:
        tc=s.groupby("Party")["Amount"].sum(); tp=(tc.max()/rev*100) if rev>0 else 0
        if tp>35: leaks.append({"title":"Customer concentration risk","sub":f"Top customer = {tp:.1f}% of revenue. Hidden business risk."})
    return leaks[:5]
...
def get_collection_wa_message(party, amount, days):
    if days>60: return f"Dear {party}, this is an urgent reminder that your payment of {fmt_inr(amount)} has been pending for over {days} days. Please settle this at the earliest to avoid disruption. Thank you."
    elif days>30: return f"Hi {party}, a gentle reminder that {fmt_inr(amount)} is overdue on your account. We'd appreciate payment at your earliest convenience. Thank you."
    else: return f"Hi {party}, quick reminder about the outstanding payment of {fmt_inr(amount)}. Request you to clear this at your earliest. Thank you for your continued business!"
...
def detect_anomalies(df):
    ...
    for cat in e["Category"].unique():
        ...
        if z>1.5: alerts.append(("danger",f"<strong>{cat}</strong>: <strong>{fmt_inr(last_val)}</strong> last month — {((last_val/mean-1)*100):.0f}% above average ({fmt_inr(mean)}/mo).",f"Check {cat} invoices immediately — could be a pricing error or one-off spike."))
    ...
    if len(sm)>=2:
        prev,last=sm.iloc[-2],sm.iloc[-1]; drop=(prev-last)/prev*100 if prev>0 else 0
        if drop>20: alerts.append(("danger",f"Revenue dropped <strong>{drop:.0f}%</strong> last month ({fmt_inr(prev)} to {fmt_inr(last)}).","Urgent: check for seasonal dip, customer loss, or collections delay. Review overdue invoices first."))
    return alerts

def gst_summary(df):
    s=df[df["Type"]=="Sales"].copy(); total_sales=s["Amount"].sum()
...
def cash_runway_warning(df):
    ...
    ae=em.tail(3).mean() if len(em)>=3 else em.mean()
    rt=(sm.iloc[-1]-sm.iloc[-3])/3 if len(sm)>=3 else 0
    next_rev=max(ar+rt,0); net=next_rev-ae
    overdue=df[df["Status"]=="Overdue"]["Amount"].sum() if "Status" in df.columns else 0
    stress="safe"
    if net<0: stress="critical"
    elif net<ae*0.1: stress="warning"
    elif overdue>ar*0.15: stress="warning"
    return {"net_next_30":net,"overdue_risk":overdue,"stress_level":stress,"avg_monthly_exp":ae}

def get_benchmarks(df, industry):
    s=df[df["Type"]=="Sales"]; e=df[df["Type"]=="Expense"]
    rev=s["Amount"].sum(); exp=e["Amount"].sum(); profit=rev-exp
    results=[]
    for metric,(your_typical,industry_avg,direction) in INDUSTRY_BENCHMARKS.get(industry,{}).items():
        if metric=="Net Margin %": yours=(profit/rev*100) if rev>0 else 0
        elif "%" in metric:
            cat_name=metric.replace(" %","").replace(" of Revenue","").split()[0]
            cat_spend=e[e["Category"].str.contains(cat_name,case=False,na=False)]["Amount"].sum() if len(e)>0 else 0
            yours=(cat_spend/rev*100) if rev>0 else your_typical
        else: yours=your_typical
        good=yours<=industry_avg if direction=="lower" else yours>=industry_avg
...
def ask_ai(question, df):
    ...
    context=(f"You are a sharp Indian SME financial advisor. Be direct, specific, use numbers, give actionable advice. Max 150 words. No disclaimers.\n"
             f"Business: Revenue={fmt_inr(rev)}, Expenses={fmt_inr(exp)}, Profit={fmt_inr(profit)} ({margin:.1f}% margin), Overdue={fmt_inr(overdue)}\n"
             f"Expenses: {json.dumps(te)}\nTop customers: {json.dumps(tc)}\nQuestion: {question}")
    import urllib.request
    api_key=st.secrets.get("ANTHROPIC_API_KEY","")
    if not api_key: return "Add ANTHROPIC_API_KEY to Streamlit secrets (Settings → Secrets) to enable AI Q&A."
    ...
    try:
        ...
        return f"AI Q&A error: {str(ex)}"


# ── SESSION STATE ──────────────────────────────────────────────────────────────

for k,v in [("df",None),("industry","agency"),("unlocked",False),("chat_history",[]),("confidence",95),("mapped_cols",[])]:
...
<div class="hero">
    ...
    <p class="hero-sub">Upload your Excel, Tally export, or bank statement. Get instant P&L, health score, overdue alerts, GST summary, and plain-English actions — no accountant needed.</p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    st.markdown("<div class='mini-note'>Works with Excel files · CSV exports · Tally exports · billing / invoice reports · bank statements</div>", unsafe_allow_html=True)
with col2:
    ...
    if st.button("Load sample data",use_container_width=True):
        ...
        st.session_state.df=gen_fn(); st.session_state.industry=ind_key
        st.session_state.confidence=95; st.session_state.mapped_cols=["Date","Amount","Type","Category","Party","Status","Invoice_No"]

if uploaded_file:
    df_up,ok,msg,conf,mapped=parse_uploaded_file(uploaded_file)
    if ok:
        st.session_state.df=df_up; st.session_state.industry="agency"
...
if st.session_state.df is not None:
    ...
    if mom:
        ...
        def dc(pct,hb=True): return "#c8ff57" if (pct>0 if hb else pct<0) else "#ff5e5e"
        def da(pct): return "▲" if pct>0 else "▼"
        cr,pr,pr_=mom["rev"]; ce,pe,pe_=mom["exp"]; cp,pp,pp_=mom["profit"]
        m1.markdown(f'<div class="metric-card blue"><div class="metric-label">Revenue — {mom["cur_month"]}</div><div class="metric-value">{fmt_inr(cr)}</div><div class="metric-sub" style="color:{dc(pr_)}">{da(pr_)} {abs(pr_):.1f}% vs {mom["prev_month"]} ({fmt_inr(pr)})</div></div>', unsafe_allow_html=True)
        m2.markdown(f'<div class="metric-card amber"><div class="metric-label">Expenses — {mom["cur_month"]}</div><div class="metric-value">{fmt_inr(ce)}</div><div class="metric-sub" style="color:{dc(pe_,False)}">{da(pe_)} {abs(pe_):.1f}% vs {mom["prev_month"]} ({fmt_inr(pe)})</div></div>', unsafe_allow_html=True)
        m3.markdown(f'<div class="metric-card {"green" if cp>=0 else "red"}"><div class="metric-label">Profit — {mom["cur_month"]}</div><div class="metric-value">{fmt_inr(abs(cp))}</div><div class="metric-sub" style="color:{dc(pp_)}">{da(pp_)} {abs(pp_):.1f}% vs {mom["prev_month"]} ({fmt_inr(abs(pp))})</div></div>', unsafe_allow_html=True)

    # Cash runway
    runway=cash_runway_warning(df)
    if runway["stress_level"]=="critical":
        st.error(f"🚨 **Cash flow warning:** Projected loss of **{fmt_inr(abs(runway['net_next_30']))}** next month. Immediate action needed — review expenses and chase collections.")
    elif runway["stress_level"]=="warning":
        st.warning(f"⚠️ **Cash flow caution:** Next 30 days look tight — projected net: **{fmt_inr(runway['net_next_30'])}**. You have **{fmt_inr(runway['overdue_risk'])}** in overdue invoices that could ease this.")
    else:
    ...
    sh=f'<div class="health-wrap"><div style="display:flex;align-items:baseline;gap:12px;"><div class="health-score" style="color:{color}">{overall}</div><div style="font-size:1.05rem;color:#7f7f92;">/ 100 — <span style="color:{color}">{label}</span></div></div><div class="health-grid">'
    for name,(sc,cl) in scores.items():
        sh+=f'<div class="health-item"><div class="health-label">{name}</div><div class="health-bar"><div class="health-fill" style="width:{sc}%;background:{cl}"></div></div><div class="health-score-small">{sc}/100</div></div>'
    sh+="</div></div>"
    ...
    st.markdown("<div class='section-sub'>Where this business needs attention first — with exact actions.</div>", unsafe_allow_html=True)
    probs=get_top_3_problems(df,industry)
    ph='<div class="problem-grid">'
    for p in probs:
        ph+=f'<div class="problem-card"><div class="problem-tag {p["severity"]}">{p["severity"].upper()}</div><div class="problem-title">{p["title"]}</div><div class="problem-text">{p["text"]}</div><div class="problem-action"><strong>Action:</strong> {p["action"]}</div></div>'
    ph+="</div>"
    ...
    # Trust
    st.markdown(f'<div class="trust-box"><div class="trust-title">Data confidence</div><div class="trust-text">Parsed with <strong>{st.session_state.confidence}%</strong> confidence. Detected fields: <strong>{", ".join(st.session_state.mapped_cols) if st.session_state.mapped_cols else "None"}</strong>. If numbers look off, download the raw CSV and check source data.</div></div>', unsafe_allow_html=True)
    ...
    tab1,tab2,tab3,tab4,tab5,tab6,tab7,tab8=st.tabs([
        ...
        "Benchmarks","GST Summary","Cash Flow Forecast","Collections & Invoices","Ask AI"])
    ...
    with tab4:
        ...
        for i,b in enumerate(get_benchmarks(df,industry)):
            col=bc1 if i%2==0 else bc2
            bar_color="#c8ff57" if b["good"] else "#ff5e5e"
            bar_pct=min(b["yours"]/max(b["benchmark"]*2,1)*100,100)
            with col:
                st.markdown(f'<div class="bench-card"><div class="bench-label">{b["metric"]}</div><div class="bench-row"><div><div class="bench-yours" style="color:{bar_color}">{b["yours"]:.1f}%</div></div><div style="text-align:right"><div class="bench-avg">Industry avg: {b["benchmark"]}%</div><div style="font-size:11px;color:{"#c8ff57" if b["good"] else "#ff7c7c"};margin-top:2px;">{"Good" if b["good"] else "Needs work"}</div></div></div><div class="bench-bar-wrap"><div class="bench-bar-fill" style="width:{bar_pct}%;background:{bar_color}"></div></div></div>', unsafe_allow_html=True)
    ...
    with tab6:
        ...
        if avg_p>0: st.success(f"Forecast looks healthy — avg projected profit {fmt_inr(avg_p)}/month.")
    ...
    with tab7:
        ...
        if collections:
            ...
            for c in collections:
                st.markdown(f'<div class="collection-item"><div><div class="collection-party">{c["party"]}</div><div class="collection-days">{c["count"]} invoice(s) · {c["days"]} days overdue</div></div><div class="collection-amt">{fmt_inr(c["amount"])}</div></div>', unsafe_allow_html=True)
                with st.expander(f"WhatsApp reminder for {c['party']}"):
                    st.text_area("Copy & send:",value=get_collection_wa_message(c["party"],c["amount"],c["days"]),height=100,key=f"wa_{c['party']}",label_visibility="collapsed")
        else:
    ...
    with tab8:
        ...
        st.markdown("<div class='section-sub'>Ask in plain English — no finance jargon needed.</div>", unsafe_allow_html=True)
        suggested=["Why did profit drop?","Which expense should I cut first?","Who owes me money?","What is hurting my cash flow?","What should I fix this month?"]
    ...
    # Export
    st.markdown("<div class='section-title'>Download & Share</div>", unsafe_allow_html=True)
    ...
    with ex2:
        ...
        st.text_area("Copy & send:",value=generate_whatsapp_summary(df),height=140,label_visibility="collapsed")
    with ex3:
    ...
    st.markdown("""
    <div class="cta-box">
        <div class="cta-title">Want this for your business every month?</div>
        <div class="cta-sub">Get monthly business summaries, finance insights, anomaly alerts, and setup support tailored to your data.</div>
    </div>""", unsafe_allow_html=True)
    ct1,ct2,ct3=st.columns([1,1,1])
    with ct1:
        if st.button("Book a Demo",use_container_width=True):
            st.info("Coming soon — drop your WhatsApp number in the chat below and we'll reach out!")
    with ct2:
        if st.button("Join Early Access",use_container_width=True):
            st.info("Coming soon — early access list opening soon!")
    with ct3:
        if st.button("Contact on WhatsApp",use_container_width=True):
            st.info("Message +91-XXXXXXXXXX on WhatsApp mentioning OpsClarity.")

else:
    st.markdown("""
    <div class="problem-grid" style="margin-top:2rem;">
        <div class="problem-card"><div class="problem-tag green">INSTANT</div><div class="problem-title">P&L in 30 seconds</div><div class="problem-text">Upload any messy file. Revenue, expenses, profit, and health score — instantly.</div></div>
        <div class="problem-card"><div class="problem-tag amber">SMART</div><div class="problem-title">Not just charts — actions</div><div class="problem-text">OpsClarity tells you exactly what's wrong and what to do about it.</div></div>
        <div class="problem-card"><div class="problem-tag red">REAL</div><div class="problem-title">Built for Indian SME pain</div><div class="problem-text">Profit leaks, overdue cash, GST, benchmarks, cash flow forecast — all in one place.</div></div>
    </div>""", unsafe_allow_html=True)
