"""
OpsClarity v3 — 10/10 Product
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
NEW in v3 (the 5 upgrades to reach 10/10):

1. 🧠 INDUSTRY INTELLIGENCE LAYER
   - Each industry gets its own lens, language, thresholds
   - Restaurant sees food cost %, clinic sees chair utilisation
   - Not generic rules — industry-specific root cause

2. 📊 BENCHMARKING ENGINE
   - "You spend 22% more than similar businesses"
   - Peer percentile ranking per category
   - Visual gauge showing where you sit

3. 🔁 LEARNING SYSTEM
   - User corrections saved to session
   - App learns which suggestions were wrong
   - Confidence scores on each insight

4. 📅 CONTINUOUS TRACKING
   - Weekly trend line per metric
   - Performance improvement graph over time
   - Alerts when metrics worsen

5. 🧩 WORKFLOW INTEGRATION
   - Task assignment (who does what by when)
   - Follow-up reminders via WhatsApp
   - Execution tracking with accountability
"""
...
.intel-conf{display:inline-block;font-size:10px;background:#F0EDE6;color:#6A6A5A;padding:2px 8px;border-radius:10px;margin-left:8px;vertical-align:middle}
...
.cl-h.red{color:#E05252;font-size:11px;font-weight:600}
.cl-h.amber{color:#D4AF37;font-size:11px;font-weight:600}
.cl-h.green{color:#4CAF50;font-size:11px;font-weight:600}
...
# ─────────────────────────────────────────────────────────────
# UPGRADE 1: INDUSTRY INTELLIGENCE LAYER
# Each industry has: specific KPIs, thresholds, language, root causes
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# UPGRADE 2: BENCHMARKING ENGINE
# ─────────────────────────────────────────────────────────────
def get_percentile(value, p25, median, p75):
    """Return 0-100 percentile position (lower = better for costs)"""
    if value <= p25:   return 10   # top 25% (best)
    if value <= median:return 35
    if value <= p75:   return 65
    return 90                      # bottom 25% (worst)
...
def render_benchmark_gauge(label, your_value, p25, median, p75, unit, n, higher_is_better=False):
    ...
    if higher_is_better: pct = 100 - pct

    if pct <= 30:   bar_color="#4CAF50"; cls="good"; verdict=f"Top performer — better than most {n} peers"
    elif pct <= 60: bar_color="#D4AF37"; cls="warn"; verdict=f"Average — room to improve vs {n} peers"
    else:           bar_color="#E05252"; cls="bad";  verdict=f"Above average cost — {n} peers do better"
...
# ─────────────────────────────────────────────────────────────
# UPGRADE 3: LEARNING SYSTEM
# ─────────────────────────────────────────────────────────────
def get_confidence_score(leak_id, corrections):
    """
    Each user correction adjusts confidence.
    corrections = {leak_id: "correct"|"wrong"|"partial"}
    """
...
def render_confidence_badge(score):
    ...
    else: color="#C0392B"; label=f"{score*100:.0f}% — verify manually"
...
# ─────────────────────────────────────────────────────────────
# UPGRADE 4: CONTINUOUS TRACKING ENGINE
# ─────────────────────────────────────────────────────────────
def compute_trends(df):
    """Returns monthly trends dict: {metric: [monthly values]}"""
    ...
    trends["revenue"] = rev_m.values.tolist()
    trends["revenue_months"] = [str(m) for m in rev_m.index]

    # Expense trend
    exp_m = expenses.groupby(expenses["Date"].dt.to_period("M"))["Amount"].sum()
    trends["expenses"] = exp_m.values.tolist()
    ...
    # Overdue trend (if status col present)
...
def render_trend_chip(label, values, positive_direction="up"):
    ...
    cls   = "up" if is_good else ("down" if not is_good else "flat")
    change = ""
    if len(values)>=2:
...
# ─────────────────────────────────────────────────────────────
# UPGRADE 5: WORKFLOW INTEGRATION
# ─────────────────────────────────────────────────────────────
...
<div class="task-card {css}">
  <div style="width:36px;height:36px;border-radius:50%;background:#F7F4EF;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">
    {"✅" if task["status"]=="done" else ("🔴" if task["severity"]=="critical" else "🟡")}
  </div>
  <div class="task-left">
        ...
        if task["status"] == "pending":
            ...
            with c2:
                if st.button("✅ Done", key=f"{key_prefix}_done_{i}"):
                    ...
                    st.rerun()
            with c3:
                wa_msg = urllib.parse.quote(f"Reminder: {task['title']} is due {task['due']}. Impact: {fmt(task['impact'])}. Please action today.")
...
def _cat(d):
    ...
    kw = [
        ...
        (["logistics","courier","freight","dispatch"],"Logistics"),
...
# ─────────────────────────────────────────────────────────────
# INTELLIGENT LEAK DETECTOR (with industry-specific context)
# ─────────────────────────────────────────────────────────────
def find_leaks(df, industry, corrections=None):
    ...
    # 1. Overdue — with industry-specific language
    if "Status" in df.columns:
        ...
        if od_amt > 10000:
            ...
            leaks.append({
                ...
                "bench":f"Healthy: overdue < 5% of revenue. Yours: {pct:.1f}%. Industry context: {intel['language']['quick_win']}",
                "action":f"Call {top_name} today. Offer 2% discount for 48-hr payment.",
                "action_sub":f"Use WhatsApp sequence. {intel['language']['quick_win']}",
                "template":f"Hi, invoice of {fmt(top_amt)} is overdue. 2% off if paid today.",
                "seqs":gen_seq(top_name,"INV-001",top_amt,"Your Business"),
            })

    # 2. Vendor overpay — with industry benchmarks
    if len(expenses)>0:
        ...
        for category in expenses["Category"].unique():
            ...
            if ep>cheapest*1.12:
                ...
                if pdata:
                    bench_line=f"Your cost: ₹{ep:,.0f}. Industry median: ₹{pdata['median']:,}{pdata['unit']} ({pdata['n']} peers). You're in the {'top' if ep>pdata['p75'] else 'average'} quartile."
                else:
                    bench_line=f"Get 3 quotes — typically 10–18% saving possible on {category}."
...
defaults = {
    ...
    "snapshots":[],   # for trend tracking over multiple uploads
}
...
with t1:
    ...
    if st.session_state.df is not None and not st.session_state.lead_captured:
        st.markdown('<div class="sw"><div class="gate-box"><div class="gate-h">Scan complete 🎯</div><div class="gate-s">Enter your WhatsApp number to see full results.</div></div>', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        cols=st.columns(4)
        with cols[0]: render_trend_chip("Revenue", trends.get("revenue",[0,0]), positive_direction="up")
        with cols[1]: render_trend_chip("Expenses", trends.get("expenses",[0,0]), positive_direction="down")
        with cols[2]: render_trend_chip("Margin", trends.get("margin",[0,0]), positive_direction="up")
        with cols[3]:
            if "overdue" in trends: render_trend_chip("Overdue", trends["overdue"], positive_direction="down")

        st.markdown("<br>", unsafe_allow_html=True)

        # ── LEAK CARDS with confidence + root cause ───────────
        st.markdown('<div class="sh">Where your money is leaking</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="ss">{intel["name"]} · {len(leaks)} issues found · Confidence scores based on your data quality</div>', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
            ...
            c1f,c2f,c3f,c4f = st.columns([3,1,1,1])
            with c2f:
                if st.button("✅ Correct", key=f"{fb_key}_c"):
                    st.session_state.corrections[leak["id"]]="correct"
                    st.rerun()
            with c3f:
                if st.button("⚠️ Partial", key=f"{fb_key}_p"):
                    st.session_state.corrections[leak["id"]]="partial"
                    st.rerun()
            with c4f:
                if st.button("❌ Wrong", key=f"{fb_key}_w"):
                    st.session_state.corrections[leak["id"]]="wrong"
                    st.rerun()

            if leak["id"]=="cash_stuck" and leak.get("seqs"):
                if st.button("📱 Collections sequence", key="bot"):
                ...
                if st.button("📋 Script", key=f"scr_{leak['id']}"):
        ...
        with pc1: st.markdown('<div class="pr-card"><div class="pr-lbl">Free</div><div class="pr-name">First Scan</div><div class="pr-amt">₹0</div><div class="pr-note">Full intelligence scan. No strings.</div><div class="pr-feat">Industry-specific insights</div><div class="pr-feat">Peer benchmarks</div><div class="pr-feat">Action tracker</div></div>', unsafe_allow_html=True)
        with pc2: st.markdown('<div class="pr-card feat"><div class="pr-lbl">Most chosen</div><div class="pr-name">Recovery Review</div><div class="pr-amt">₹2,999</div><div class="pr-note">60-min call. 30-day plan. Full ROI tracking.</div><div class="pr-feat">Founder call</div><div class="pr-feat">Vendor sourcing</div><div class="pr-feat">Monthly ROI report</div><div class="pr-feat">CA coordination</div></div>', unsafe_allow_html=True)
        with pc3: st.markdown('<div class="pr-card"><div class="pr-lbl">CA Firms</div><div class="pr-name">Partner Plan</div><div class="pr-amt">₹1,999/mo</div><div class="pr-note">50 client seats. White-label. ₹500/client earn.</div><div class="pr-feat">Client dashboard</div><div class="pr-feat">Branded reports</div><div class="pr-feat">ROI proof per client</div></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
# TAB 2 — INTELLIGENCE (Benchmarks + Industry Context)
# ═══════════════════════════════════════════════════════════════
with t2:
    st.markdown('<div class="sw">', unsafe_allow_html=True)
    if st.session_state.df is None:
        ...
        st.markdown(f'<div class="ss">What the data means specifically for your industry. Not generic advice — {intel["name"]}-specific context.</div>', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        # Benchmarking gauges per category
        ...
        if len(expenses)>0 and revenue>0:
            ...
            for cat, pdata in peer_stats.items():
                st.markdown(f'<div class="bench-wrap"><div class="bench-label">{pdata["label"]} — industry range</div><div style="font-size:13px;color:#6A6A5A">Best 25%: {pdata["p25"]}{pdata["unit"]} · Median: {pdata["median"]}{pdata["unit"]} · Worst 25%: {pdata["p75"]}{pdata["unit"]} ({pdata["n"]} peers)</div></div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
# TAB 3 — ACTION BOARD (Workflow Integration)
# ═══════════════════════════════════════════════════════════════
with t3:
    st.markdown('<div class="sw">', unsafe_allow_html=True)
...
with t4:
    ...
    if not st.session_state.tasks:
        ...
        if   roi_x>=10: verdict=f"Exceptional — every ₹1 on OpsClarity returned ₹{roi_x:.0f}"
        elif roi_x>=3:  verdict=f"Strong ROI — {roi_x:.1f}x return on investment"
        elif roi_x>=1:  verdict=f"Positive ROI — complete pending actions for full impact"
        else:           verdict=f"{sum(1 for t in st.session_state.tasks if t['status']=='pending')} actions pending — complete them to see returns"
...
<div class="roi-wrap">
  ...
  <div class="roi-grid">
    <div class="roi-cell"><div class="roi-cell-lbl">Fee Paid</div><div class="roi-cell-val">{fmtx(total_fees)}</div></div>
...
""", unsafe_allow_html=True)
        ...
        # Download
        lines=["="*50,f"ROI REPORT — OpsClarity",f"Business: {biz}",
               f"Period: {st.session_state.months} month(s) | {datetime.now().strftime('%d %b %Y')}","="*50,
               f"Fee paid:    {fmtx(total_fees)}",f"Saved:       {fmt(total_saved)}",
        ...
        with d2:
            msg=urllib.parse.quote(f"OpsClarity ROI: Paid {fmtx(total_fees)}, Saved {fmt(total_saved)}, {roi_x:.1f}x return. {verdict}")
...
""", unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="ca-card dark"><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1rem"><div><div class="ca-lbl">Clients</div><div style="font-family:\'DM Serif Display\',serif;font-size:1.4rem;color:#F7F4EF">{len(portfolio)}</div></div><div><div class="ca-lbl">Leaks found</div><div style="font-family:\'DM Serif Display\',serif;font-size:1.4rem;color:#D4AF37">{fmt(total_pl)}</div></div><div><div class="ca-lbl">Actually saved</div><div style="font-family:\'DM Serif Display\',serif;font-size:1.4rem;color:#4CAF50">{fmt(total_ps)}</div></div><div><div class="ca-lbl">Recovery rate</div><div style="font-family:\'DM Serif Display\',serif;font-size:1.4rem;color:#D4AF37">{total_ps/total_pl*100:.0f}%</div></div></div><div style="border-top:1px solid rgba(255,255,255,0.08);padding-top:.75rem">{"".join([f\'<div class="cl-row"><div><div class="cl-name">{c["name"]}</div><div class="cl-meta">{c["city"]} · {c["ind"].title()}</div></div><div style="text-align:right"><div class="cl-amt">{fmt(c["leak"])} found</div><div style="font-size:11px;color:#4CAF50">{fmt(c["saved"])} saved</div></div><div class="cl-h {c["health"]}">{"🔴 Act" if c["health"]=="red" else "🟡 Watch" if c["health"]=="amber" else "🟢 OK"}</div></div>\' for c in portfolio])}</div></div>', unsafe_allow_html=True)

    for q,a in [
        ("My clients won't share data with a third-party","You upload — they never see OpsClarity. Report is branded as your firm's work."),
        ("How does the ROI report work?","Each action taken is logged. Monthly report = fee paid vs money saved. Your retention weapon."),
        ("Will this replace CAs?","No. Every tax finding says 'verify with your CA'. We surface the work — you do it and bill for it."),
        ("What if the numbers are wrong?","You verify before sharing. Your professional judgement is the product."),
    ]:
